    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="p-5 rounded-lg shadow bg-white">
                ...
                <a class="block w-[200px] text-center text-white px-3 py-2 bg-green-500 rounded-md m-4" href="{{ route('item.create') }}">Tambah Item</a>
                @endif
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $item->id_item }}</th>
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $item->nama_item }}</th>
                            <td class="px-6 py-4">{{ $item->uom }}</td>
                            <td class="px-6 py-4">{{ $item->harga_beli }}</td>
                            <td class="px-6 py-4">{{ $item->harga_jual }}</td>
                            <td class="px-6 py-4">
                                <a href="{{ route('item.edit', $item) }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Edit</a>
                                @if(Auth::user()->level->level != 'Manager')
